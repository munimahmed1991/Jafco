...
Public Class frmFinalNoticeService
    ...
    Private Sub tdbGridLetters_AfterColUpdate(ByVal sender As Object, ByVal e As C1.Win.C1TrueDBGrid.ColEventArgs) Handles tdbGridLetters.AfterColUpdate
        If tdbGridLetters.Columns("Generate").Value = 1 Then
            tdbGridLetters.Columns("GenerateDate").Value = Format(txtGenerateDate.Value, "MM/dd/yyyy")
        Else
    ...
    Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        ...
        For Each row1 As DataRow In dbGrid.cDataTableCollection(0).Select
            If RemoveNull(row1!Generate, 0) = 1 Then
                dbPreviousLetter = New dbOperations("Select EntryDate as EntryDate From tblLetters Where Unit = '" & row1!Unit & "' and Upper(Substring(LetterType,1,18)) = 'DUE SERVICE LETTER' Order By EntryDate Desc")
                ...
                cNextNo = NextSerial("EntryNo", "tblLetters", "000", cPrefix + "/" + Trim(row1!UnitNumber) + "/", "Unit = '" & row1!Unit & "'")

                cSql = "Insert Into tblLetters(EntryNo,EntryDate,LetterType,Customer,Customer2,Unit,Project,GenerateSystemDate,PreviousLetters) Values('" & cNextNo & "','" & row1!GenerateDate & "','Due Service Letter 5','" & row1!Customer & "','" & row1!Customer2 & "','" & row1!Unit & "','" & row1!Project & "','" & Format(Today.Date, "MM/dd/yyyy") & "','" & cPreviousLetter & "')"
        ...
        MsgBox("Payment Due for Services Letter Generated Successfully")
