...
/****** Object:  StoredProcedure [dbo].[RecoveryReport]    Script Date: 4/2/2016 4:33:37 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER Procedure [dbo].[RecoveryReport] @DateFrom DateTime,@DateTo DateTime,@Project Varchar(4),@UnitType Varchar(10),@UnitNumber Varchar(14),@ReportType Varchar(50),@PaymentStatus Varchar(20),@Penalty Numeric(9) = 0 ,@PaymentType Varchar(4) = '',@CustomerType Varchar(50) = 'Both'
...
Select	tblUnitFile.UnitType,Mst.Project,Mst.Unit,tblUnitFile.UnitNumber,Max(IsNull(Mst.CashAmount,0)) as CashAmount,Max(IsNull(Mst.ExtraCharges,0)) + Max(IsNull(Mst.AddOtherCharges,0)) as ExtraCharges,Max(IsNull(Mst.LoanAmount,0)) as LoanAmount,0 as DueTillDate,0 as AmountReceived,0 as AmountReceivedLoan,0 as Ontime,0 as Late,0 as TotalPayment,0 as Unofficial
Into	#Temp1
...
Select	Project,UnitType,Unit,UnitNumber,
		...
		Sum(DueTillDate) - Sum(AmountReceived) - Sum(AmountReceivedLoan) as BalanceTillDate,Max(OnTime) as OnTime,Max(Late) as Late,Max(TotalPayment) as TotalPayment,Sum(UnOfficial) as Unofficial
Into	#Temp2
From	#Temp1
Group By Project,Unit,UnitNumber,UnitType

Select	Mst.Project,tblUnitFile.UnitType,Mst.Unit,tblUnitFile.UnitNumber,Sum(Amount) as DueforThePeriod,0 as ReceivedForThePeriod,0 as Unofficial
Into	#Temp3
...
Group By Mst.Project,Mst.Unit,tblUnitFile.UnitNumber,tblUnitFile.UnitType
Union All
Select	Mst.Project,tblUnitFile.UnitType,Mst.Unit,tblUnitFile.UnitNumber,0 as DueforThePeriod,Sum(ReceiptAmount) as ReceivedForThePeriod,0 as Unofficial
...
Select	Project,UnitType,Unit,UnitNumber,Sum(DueforthePeriod) as DueForThePeriod,Sum(ReceivedForThePeriod) as ReceivedForThePeriod,Sum(DueforThePeriod) - Sum(ReceivedForThePeriod) as BalanceforThePeriod,0 as UnOfficial
Into	#Temp4
From	#Temp3
GRoup By Project,UnitType,Unit,UnitNumber

Select	Project,UnitType,Unit,UnitNumber,
		CashAmount,LoanAmount,ExtraCharges,DueTillDate,AmountReceived,AmountReceivedLoan,BalanceTillDate,
		0 as DueForThePeriod,0 as ReceivedForThePeriod,0 as BalanceforThePeriod,OnTime,Late,TotalPayment,UnOfficial
Into	#Temp5
From	#Temp2
Union All
Select	Project,UnitType,Unit,UnitNumber,
		0 as CashAmount,0 as LoanAmount,0 as ExtraCharges,0 as DueTillDate,0 as AmountReceived,0 as AmountReceivedLoan,0 as BalanceTillDate,
		DueForThePeriod,ReceivedForThePeriod,BalanceforThePeriod,0 as OnTime,0 as Late,0 as TotalPayment,0 as UnOfficial
From	#Temp4

Select	#Temp5.Project,#Temp5.UnitType,#Temp5.Unit,#Temp5.UnitNumber,
		Sum(#Temp5.CashAmount) as CashAmount,Sum(ExtraCharges) as ExtraCharges,Sum(#Temp5.LoanAmount) as LoanAmount,Sum(#Temp5.DueTillDate) as DueTillDate,Sum(#Temp5.AmountReceived) as AmountReceived,Sum(#Temp5.AmountReceivedLoan) as AmountReceivedLoan,Sum(#Temp5.BalanceTillDate) as BalanceTillDate,
		...
		Max(Cast(OnTime as Numeric(9,2))) as OnTime,Max(Cast(Late as Numeric(9,2))) as Late,Sum(UnOfficial) as UnOfficial
Into	#Temp6
...
Select	#Temp6.Unit,UnitType,tblUnitType.Name as UnitTypeName,UnitNumber,ReceiptDate as LastReceiptDate,
		...
		OnTime,Late,Case When IsNull(OnTime,0) > 0 Then Round((OnTime / (OnTime + Late)) * 100,0) Else 0 End as OnTimePer,UnOfficial as UnOfficial
Into	#Temp8
...
Select	#Temp8.*,
		tblAgent.Name as AgentName,
		tblAgentCompany.Name as AgentCompany,
		Case When OnTimePer >= 80 Then 'Excellent' Else Case When OnTimePer >= 60 and OnTimePer < 80 Then 'Good' Else 'Poor' End End as PaymentStatus,
		RecoveryPerCash + RecoverPerLoan as RecoverPer,
		AmountReceivedCash + AmountReceivedLoan as AmountReceived,
		Discount,
		TotalRevenue as NetSalesValue
Into	#Temp7 
From	#Temp8,tblReservationMst 
Left Outer Join tblAgent Left Outer Join tblAgentCompany On tblAgent.AgentCompany = tblAgentCompany.Code On tblAgent.Code = tblReservationMst.Agent
Where	#Temp8.Unit = tblReservationMst.Unit 
...
if @ReportType = 'Report' 
	Begin
		Select #Temp7.*,#LastLetter.LetterType From #Temp7 Left Outer Join #LastLetter On #LastLetter.Unit = #Temp7.Unit Where PaymentStatus = Case When @PaymentStatus <> 'All' Then @PaymentStatus Else PaymentStatus End Order BY UnitType
	ENd

if @ReportType = 'Grid'
	Begin
		Select	UnitTypeName,UnitNumber,AgentCompany,AgentName,LastReceiptDate,
				CashAmount as CashAmount,Case When @PaymentType = '' Then LoanAmount Else 0 End as LoanAmount,ExtraCharges,Discount,NetSalesValue,DueTillDate,AmountReceivedCash,AmountReceivedLoan,BalanceTillDate,RecoveryPerCash,RecoverPerLoan,RecoverPer,
				DueforthePeriod,ReceivedForThePeriod,BalanceForThePeriod,RecoveryPerPeriod,UnOfficial,Case When IsNull(NetSalesValue,0) <> 0 and UnOfficial <> 0 Then (UnOfficial / IsNull(NetSalesValue,0)) * 100 Else 0 End as UnofficialPer,
				OnTime,Late,Case When OnTimePer >= 80 Then 'Excellent' Else Case When OnTimePer >= 60 and OnTimePer < 80 Then 'Good' Else 'Poor' End End as PaymentStatus,#LastLetter.LetterType
		From	#Temp7  Left Outer Join #LastLetter On #LastLetter.Unit = #Temp7.Unit
		Order By UnitType
	End
